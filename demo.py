...
# 动态算出num_encoder_symbols和num_decoder_symbols
max_token_id = wordToken.load_file_list(['./samples/question.txt', './samples/answer.txt'], min_freq)
# max_token_id = wordToken.load_file_list(['./samples/ssa.txt'], min_freq)
...
def get_samples(train_set, batch_num):
    ...
    :return:
        参数encoder_inputs是一个list，list中每一项是1D的Tensor，这个Tensor的shape是[batch_size]，
